...
<body style="background-color: #ffffff;">
  <form name="managecontentfrm" id="managecontentfrm" method="post" action="/shade-o-matic/content.do" enctype="multipart/form-data">
	<div id="popupwindow">
		...
		<%if(itemList != null && itemList.size() > 0){
			...
			while(iterator.hasNext()){
				...
				if(item.getType().equals(Constants.TEXT_BOX)){%>
					<div id="popuplabel"><%=item.getName()%> </div> <input type="text" name="<%=item.getName()%>" class="popuptextbox" size="40" value="<%=TextUtils.getMapValue(content, item.getName())%>">
				...
				<%}else if(item.getType().equals(Constants.FILE)){%>
					...
					<div id="popuplabel"></div>	<div id="popuptext"><%=TextUtils.getItemSummary(item)%></div>
					<input type="hidden" name="htm_<%=item.getName()%>" value="<%=TextUtils.getMapValue(content, item.getName())%>">
				<%}else{%>
					<div id="popuplabel"><%=item.getName()%> </div> 
					<div id="ckeditor">
						<textarea cols="50" id="editor1" name="<%=item.getName()%>" rows="2"><%=TextUtils.getMapValue(content, item.getName())%></textarea>
	...
	<input type="hidden" name="<%=Constants.CONTENT_ID%>" id="<%=Constants.CONTENT_ID%>" value="<%=TextUtils.getText(request.getAttribute(Constants.CONTENT_ID))%>"/>
  </form>		
</body>
</html>	